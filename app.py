...
# 🌟 5. [독립 프래그먼트 1] 사이드바 전용 실시간 점수판 갱신 바구니
@st.fragment
def run_sidebar_metrics_fragment():
    yield_metric_holder = st.empty()
    progress_bar_holder = st.empty()
    progress_text_holder = st.empty()
    
    cur_yield = get_lot_yield()
    prog = (st.session_state.scan_idx / len(st.session_state.valid_coords)) * 100 if len(st.session_state.valid_coords) > 0 else 0
...
# 가이드대로 함수를 호출할 때 with st.sidebar 안에서 프래그먼트를 구동시켜 문법 준수 처리
with st.sidebar:
    run_sidebar_metrics_fragment()


# 🌟 6. [독립 프래그먼트 2] 메인 대시보드 및 실시간 웨이퍼 맵 가동 엔진 바구니
@st.fragment
def run_main_dashboard_engine():
            ...
            while st.session_state.scan_idx < len(st.session_state.valid_coords):
                ...
                if current_state == -1:
                    ...
                    if np.random.rand() > fail_rate:
                        ...
                        # 맵 세부 갱신 (네트워크 스트리밍 강제)
                        update_map_display()
                        
                        # 중요: 화면 전체 리프레시 대신 사이드바 프래그먼트 영역만 재호출 유도
                        st.slots_refresh_signal = True 
